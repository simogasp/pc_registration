...
class TestAxisAngleConversion:
    ...
    def test_arbitrary_rotation(self):
        ...
        assert np.allclose(axis, axis_input, atol=1e-6) or np.allclose(axis, -axis_input, atol=1e-6), \
...
class TestTransformationError:
    ...
    def test_combined_error(self):
        ...
        assert isinstance(trans_err, (float, np.floating)), "Translation error should be a float"
